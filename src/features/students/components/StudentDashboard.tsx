...
import { generateAIFeedback, generateFallbackFeedback } from '@/services/aiFeedbackService';
...
export const StudentDashboard: React.FC<Props> = ({ session, onLogout }) => {
    ...
    const finishQuiz = async () => {
        ...
        // KATEGORISASI ASESMEN DIAGNOSTIK MIMPHY (Rendah, Sedang, Tinggi)
        let abilityCategory: AbilityLevel = AbilityLevel.MEDIUM;
        if (finalScore >= 85) {
            abilityCategory = AbilityLevel.HIGH; // 'Tinggi'
        } else if (finalScore < 60) {
            abilityCategory = AbilityLevel.BASIC; // 'Rendah'
        } else {
            abilityCategory = AbilityLevel.MEDIUM; // 'Sedang'
        }

        // Analisis Indikator (Dikuasai vs Perlu Diperbaiki)
        const indicatorMap = new Map<string, { total: number; correct: number }>();
        activePacket.questions.forEach((q, idx) => {
            const indName = (q.indicator && q.indicator.trim()) ? q.indicator.trim() : (q.topic || `Indikator ${idx + 1}`);
            const existing = indicatorMap.get(indName) || { total: 0, correct: 0 };
            existing.total += 1;
            if (userAnswers[idx] === q.correctIndex) {
                existing.correct += 1;
            }
            indicatorMap.set(indName, existing);
        });

        const masteredIndicators: string[] = [];
        const unmasteredIndicators: string[] = [];
        const indicatorStats: Array<{ indicator: string; correct: number; total: number }> = [];

        indicatorMap.forEach((val, ind) => {
            indicatorStats.push({ indicator: ind, correct: val.correct, total: val.total });
            if (val.correct / val.total >= 0.7) {
                masteredIndicators.push(ind);
            } else {
                unmasteredIndicators.push(ind);
            }
        });

        // Rekomendasi Modul yang Tersedia di Paket
        const availableModuleTitles: string[] = [];
        if (activePacket.learningMaterials) {
            activePacket.learningMaterials.forEach(m => availableModuleTitles.push(m.title));
        }
        if (activePacket.modules) {
            [...activePacket.modules.basic, ...activePacket.modules.medium, ...activePacket.modules.high].forEach(m => {
                if (!availableModuleTitles.includes(m.title)) availableModuleTitles.push(m.title);
            });
        }

        // Pemanggilan Gemini API via Backend / Edge Function (Maksimal 1 Request per submit)
        let aiFeedback;
        try {
            aiFeedback = await generateAIFeedback({
                score: finalScore,
                abilityLevel: abilityCategory,
                packetName: activePacket.name,
                testTopic: activePacket.questions[0]?.topic || "Fisika SMA",
                masteredIndicators,
            ...
            console.error("AI Feedback error, fallback used:", e);
        ...
        return (
            ...
            >
                <div className="max-w-4xl mx-auto mt-6 md:mt-10 p-6 space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-500 pb-20 flex-1 w-full">
                                ...
                                <div className="bg-gradient-to-br from-orange-50 to-amber-50 dark:from-slate-700 dark:to-slate-800 p-6 rounded-3xl min-w-[220px] border border-orange-100 dark:border-slate-600">
                                    <div className="mb-2">
                                        {result?.abilityLevel === 'Menunggu Finalisasi' || !result?.abilityLevel ? (
                                            ...
                                            <span className={`text-3xl font-black ${result?.abilityLevel === AbilityLevel.HIGH || String(result?.abilityLevel) === 'Tinggi' ? 'text-green-600 dark:text-green-400' :
                    ...
                    {(() => {
                        const displayFeedback = result?.aiFeedback || (result ? generateFallbackFeedback({
                            score: result.score,
                            abilityLevel: String(result.abilityLevel || (result.score >= 85 ? 'Tinggi' : result.score < 60 ? 'Rendah' : 'Sedang')),
                            packetName: activePacket?.name || 'Asesmen Diagnostik',
                            testTopic: activePacket?.questions[0]?.topic || 'Fisika SMA',
                            masteredIndicators: [],
                            unmasteredIndicators: [],
                            indicatorStats: []
                        ...
                        return (
                            <div className="bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl p-8 md:p-10 rounded-[2.5rem] shadow-xl border border-stone-200 dark:border-slate-700 text-left space-y-6 animate-in fade-in slide-in-from-bottom-6 duration-500">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-stone-100 dark:border-slate-700 pb-5">
                                    <div className="flex items-center space-x-3">
                                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-red-500 to-orange-500 text-white flex items-center justify-center shadow-lg shadow-orange-500/20">
                                            <BrainCircuit className="w-6 h-6" />
                                        </div>
                                        <div>
                                            <h3 className="font-extrabold text-xl text-stone-800 dark:text-white flex items-center">
                                                AI Diagnostic Feedback
                                                <Sparkles className="w-4 h-4 ml-2 text-amber-500 fill-amber-500" />
                                            </h3>
                                            <p className="text-xs text-stone-400 dark:text-slate-400 font-medium">
                                                Analisis diagnostik mendalam & rekomendasi pembelajaran otomatis
                                            </p>
                                        </div>
                                    </div>
                                    <span className={`inline-flex items-center text-xs font-bold px-3 py-1.5 rounded-full border ${
                                        displayFeedback.generatedBy === 'gemini'
                                            ? 'bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/30 dark:to-indigo-900/30 text-purple-700 dark:text-purple-300 border-purple-200 dark:border-purple-800'
                                            : 'bg-stone-100 dark:bg-slate-700 text-stone-600 dark:text-slate-300 border-stone-200 dark:border-slate-600'
                                    }`}>
                                        {displayFeedback.generatedBy === 'gemini' ? '✨ Gemini AI Engine' : '⚡ Diagnostic Mimphy Engine'}
                                    </span>
                                </div>

                                {/* 1. Ringkasan Hasil */}
                                <div className="bg-gradient-to-r from-red-50/70 to-orange-50/70 dark:from-slate-700/50 dark:to-slate-800/50 p-5 rounded-2xl border border-red-100 dark:border-slate-700">
                                    <h4 className="text-sm font-bold text-red-900 dark:text-red-300 mb-1.5 flex items-center">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    {/* 2. Kekuatan Siswa */}
                                    <div className="bg-emerald-50/60 dark:bg-emerald-950/20 p-5 rounded-2xl border border-emerald-100 dark:border-emerald-900/30 space-y-3">
                                        <h4 className="text-sm font-bold text-emerald-900 dark:text-emerald-300 flex items-center">
                                            <CheckCircle2 className="w-4 h-4 mr-2 text-emerald-600 dark:text-emerald-400" /> Kekuatan Siswa
                                        </h4>
                                    ...
                                    <div className="bg-amber-50/60 dark:bg-amber-950/20 p-5 rounded-2xl border border-amber-100 dark:border-amber-900/30 space-y-3">
                                        <h4 className="text-sm font-bold text-amber-900 dark:text-amber-300 flex items-center">
                                            <Flag className="w-4 h-4 mr-2 text-amber-600 dark:text-amber-400" /> Area yang Perlu Diperbaiki
                                        </h4>
                                        <ul className="space-y-2">
                                ...
                                <div className="bg-blue-50/60 dark:bg-blue-950/20 p-5 rounded-2xl border border-blue-100 dark:border-blue-900/30 space-y-3">
                                    <h4 className="text-sm font-bold text-blue-900 dark:text-blue-300 flex items-center">
                                        <BookOpen className="w-4 h-4 mr-2 text-blue-600 dark:text-blue-400" /> Saran Belajar Konkret
                                    </h4>
                                    <ul className="space-y-2">
                                ...
                                <div className="bg-purple-50/60 dark:bg-purple-950/20 p-5 rounded-2xl border border-purple-100 dark:border-purple-900/30 space-y-3">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        {displayFeedback.recommendedModules.map((modTitle, i) => (
                                            <div key={i} className="bg-white dark:bg-slate-800 p-3 rounded-xl border border-purple-100 dark:border-purple-900/40 text-xs font-bold text-purple-900 dark:text-purple-200 flex items-center">
